                ...
                <table class="table-list">
                    ...
                    try {
                        ...
                        if ($result) {
                            ...
                            $sql_proposta = $pdo->prepare("SELECT p.idProposta, p.fk_idBaba, p.fk_Ppk_idUsuario, p.data, p.turno, u.nome AS nomeSolicitante, 
                                pa.descricao, u.telefone, u.email, p.dataAceite, p.dataPronto
                                FROM proposta p
                                LEFT JOIN pais pa ON p.fk_Ppk_idUsuario = pa.pk_idUsuario
                                LEFT JOIN usuario u ON pa.pk_idUsuario = u.idUsuario
                                WHERE p.fk_idBaba = :idBaba AND p.estado = 1 AND p.dataPronto IS NULL
                                ");
                            ...
                            if (count($propostas) > 0) {
                                foreach ($propostas as $proposta):
                                    if ($proposta['dataPronto'] !== null) {
                                        continue; // Ignora a proposta se dataPronto não for nulo
                                    }
                                ...
                                echo "<tr><td colspan='4' class='list-body-content'>Nenhuma proposta encontrada...</td></tr>";
                            }
                        } else {
